  ...
  return (
    <>
      <PageHeader
        ...
        title={<>Halo Rangga, kekayaanmu <em className="italic text-amber-deep">tumbuh</em> bulan ini.</>}
        action={
      ...
      <div className="stagger mb-[18px] grid grid-cols-1 gap-[18px] sm:grid-cols-2 lg:grid-cols-3">
        <StatCard
          label="Total Aset" value="Rp133,0jt" delta="▲ 6,8%" sub="vs bulan lalu"
          icon={<TrendingUp size={19} />} iconClass="bg-brand-green/10 text-brand-green"
        />
        <StatCard
          label="Total Utang" value="Rp25,0jt" delta="▼ 7,4%" sub="berkurang, bagus!"
          icon={<TrendingDown size={19} />} iconClass="bg-brand-red/10 text-brand-red"
        />
        <StatCard
          dark label="Kekayaan Bersih" value="Rp108,0jt" delta="▲ 9,1%" sub="tren positif 6 bulan"
          icon={<Star size={19} />} iconClass="bg-amber/20 text-amber"
        />
      </div>
      ...
      <div className="mb-[18px] grid grid-cols-1 gap-[18px] lg:grid-cols-[1.7fr_1fr]">
        <Card>
          <div className="mb-[18px] flex items-center justify-between">
            <div>
              ...
              <div className="text-[13.5px] text-ink-dim dark:text-slate-400">Aset vs kewajiban, 8 bulan terakhir</div>
            </div>
            <span className="rounded-md bg-brand-green/10 px-2 py-1 text-[11.5px] font-semibold text-brand-green">Sehat</span>
          </div>
          <NetWorthChart />
        </Card>
        ...
        <Card>
          ...
          <div className="mb-2 text-[13.5px] text-ink-dim dark:text-slate-400">Sebaran kekayaanmu</div>
      ...
      <div className="grid grid-cols-1 gap-[18px] lg:grid-cols-[1fr_1.4fr]">
        <Card>
          ...
          <div className="mb-[18px] text-[13.5px] text-ink-dim dark:text-slate-400">Berdasarkan rasio aset & utang</div>
        ...
        <Card>
          <div className="mb-1.5 flex items-center justify-between">
            ...
            <Link href="/cash" className="text-[13.5px] font-semibold text-amber-deep hover:underline">
              Lihat semua →
            </Link>
          </div>
          {goals.map((g) => {
            ...
            return (
              <div key={g.id} className="flex items-center gap-3.5 border-b border-black/5 py-[15px] last:border-0 dark:border-white/5">
                <div className="grid h-[42px] w-[42px] shrink-0 place-items-center rounded-xl text-[19px]" style={{ background: `${g.color}1a` }}>
                  {g.emoji}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="mb-[7px] flex justify-between gap-2.5">
                    ...
                    <div className="text-[13px] text-ink-dim dark:text-slate-400">
                      {rpShort(g.usedAmount)} <span className="text-ink-faint">/ {rpShort(g.targetAmount)}</span>
                    </div>
                  </div>
                  <ProgressBar value={p} color={g.color} />
                </div>
                <div className="w-[42px] text-right text-[14px] font-bold" style={{ color: g.color }}>{p}%</div>
              </div>
            );
          })}
        </Card>
