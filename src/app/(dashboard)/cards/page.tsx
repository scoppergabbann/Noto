...
export default function CardsPage() {
  return (
    <>
      <PageHeader
        eyebrow="Credit Card"
        title={<>Kartu kreditmu, <em className="italic text-amber-deep">terpantau</em>.</>}
        action={
          ...
          return (
            <div key={c.id}>
              ...
              >
                ...
                <div className="relative z-10 text-[18px] font-medium tracking-[.16em] tabular-nums">
                  •••• •••• •••• {c.last4}
                </div>
                <div className="relative z-10 flex items-end justify-between">
                  <div>
                    <div className="text-[10.5px] uppercase tracking-[.1em] opacity-65">Sisa Tagihan</div>
              ...
              <Card className="-mt-3.5 rounded-t-none pt-7">
                <div className="mb-[9px] flex items-center justify-between">
                  ...
                  <span className={`rounded-md px-2 py-1 text-[11.5px] font-semibold ${warn ? "bg-brand-red/10 text-brand-red" : "bg-brand-green/10 text-brand-green"}`}>
                    {util}% {warn ? "· tinggi ⚠️" : "· aman"}
                  </span>
                </div>
                <ProgressBar value={util} color={warn ? "#e0524a" : "#1f9e6f"} height={10} />
                <div className="mt-4 grid grid-cols-3 gap-2.5">
                  <div><div className="text-[11.5px] text-ink-dim dark:text-slate-400">Pemakaian</div><div className="text-[14px] font-semibold">{rpShort(c.spent)}</div></div>
                  <div><div className="text-[11.5px] text-ink-dim dark:text-slate-400">Dibayar</div><div className="text-[14px] font-semibold text-brand-green">{rpShort(c.paid)}</div></div>
                  <div><div className="text-[11.5px] text-ink-dim dark:text-slate-400">Tersedia</div><div className="text-[14px] font-semibold">{rpShort(c.limit - c.spent)}</div></div>
                </div>
                {warn && (
                  <div className="mt-3.5 rounded-xl bg-brand-red/10 px-3.5 py-2.5 text-[12.5px] font-medium text-brand-red">
                    ⚠️ Pemakaian di atas 70%. Pertimbangkan bayar Rp{suggested.toFixed(1).replace(".", ",")}jt untuk turun ke zona aman.
                  </div>
                )}
              </Card>
            </div>
